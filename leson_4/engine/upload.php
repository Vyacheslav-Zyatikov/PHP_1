...
// Директория куда будут загружаться файлы.
    $pathBig = dirname(__DIR__) . '/public/gallery_img/big/';
    $pathSmall = dirname(__DIR__) . '/public/gallery_img/small/';
        ...
        foreach ($files as $file) {
            ...
            if (!empty($file['error']) || empty($file['tmp_name'])) {
                ...
                if (empty($name) || empty($parts['extension'])) {
                    ...
                    $i = 0;
                    $prefix = '';
                    while (is_file($pathBig . $parts['filename'] . $prefix . '.' . $parts['extension'])) {
                        $prefix = '(' . ++$i . ')';
                    }
                    $name = $parts['filename'] . $prefix . '.' . $parts['extension'];

                    // Перемещаем файл в директорию.
                    if (move_uploaded_file($file['tmp_name'], $pathBig . $name)) {
                        // Далее можно сохранить название файла в БД и т.п.
                        $image = new SimpleImage();
                        $image->fromFile($pathBig . $name);
                        $image->resize(150);
                        $image->toFile($pathSmall . $name);
